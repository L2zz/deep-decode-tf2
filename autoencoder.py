...
def train_step(input):
    ...
    for batch_input in batch_inputs:
        with tf.GradientTape() as g:
            ...
            sig_arr = np.array(sig_arr).reshape(len(batch_input), INPUT, 1)
            gen_sig = np.array(gen_signal(epc_arr)).reshape(
                len(batch_input), INPUT, 1)
            pred = model(sig_arr, is_training=True)
            loss = tf.reduce_mean(tf.square(pred - gen_sig))
...
def test_step(input):
    ...
    for batch_input in batch_inputs:
        ...
        for fn in batch_input:
            ...
            sig_arr = np.array(sig_arr).reshape(len(batch_input), INPUT, 1)
            pred = model(np.array(sig_arr[fn])).numpy()
            pred = pred.reshape(len(batch_input), INPUT)
